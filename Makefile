...
lint_diff format_diff: PYTHON_FILES=$(shell git diff --relative=libs/partners/vdms --name-only --diff-filter=d master | grep -E '\.py$$|\.ipynb$$')
lint_package: PYTHON_FILES=langchain_vdms
...
check_imports: $(shell find langchain_vdms -name '*.py')
